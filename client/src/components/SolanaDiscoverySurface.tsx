...
import type { SolanaDiscoveryProfile, SolanaDiscoveryRow } from "@/solana/identityTypes";
...
export function SolanaDiscoverySurface({ profiles, skills, walletAddress }: Props) {
  ...
  const topProfiles = useMemo(
    () => [...profiles].sort((a, b) => b.discoveryScoreBps - a.discoveryScoreBps).slice(0, 8),
    [profiles]
  );
    ...
    <section className="rounded-3xl border border-cyan-500/20 bg-black/30 p-5 md:p-6">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.4fr_0.6fr]">
        <div className="space-y-3">
          {rankedSkills.length ? (
            rankedSkills.map(skill => (
              <article key={skill.skillAddress} className="rounded-2xl border border-cyan-500/20 bg-black/40 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.14em] text-cyan-400">{skill.slug}</div>
                    <h4 className="mt-1 text-lg font-semibold text-cyan-100">{skill.name}</h4>
                  </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-300">
                  ...
                  <span className="rounded-full border border-slate-600 px-2 py-0.5">{skill.category}</span>
                  <span className="rounded-full border border-slate-600 px-2 py-0.5">{skill.language}</span>
                </div>
                <div className="mt-2 flex flex-wrap gap-2">
                  {skill.tags.map(tag => (
                    <span key={tag} className="rounded-full border border-cyan-800/70 px-2 py-0.5 text-xs text-cyan-200">
                      {tag}
                    </span>
                  ))}
                </div>
        ...
        <aside className="space-y-3">
          ...
          <div className="rounded-2xl border border-cyan-500/20 bg-black/40 p-4">
            <div className="text-xs uppercase tracking-wider text-cyan-400">Which wallets have real usage?</div>
            <div className="mt-3 space-y-3">
              {topProfiles.map(profile => (
                <div key={profile.walletAddress} className="border-t border-slate-700/80 pt-3 first:border-t-0 first:pt-0">
                  <div className="text-sm font-semibold text-cyan-100">
                    {profile.walletAddress.slice(0, 4)}…{profile.walletAddress.slice(-4)}
                  </div>
                  <div className="mt-1 text-xs text-slate-300">
                    trust {pctFromBps(profile.trustScoreBps)} · discovery {pctFromBps(profile.discoveryScoreBps)}
                  </div>
                  <div className="mt-1 text-xs text-slate-400">
                    usage {profile.usageCount} · published {profile.publishedVersionCount} versions
                  </div>
                </div>
              ))}
              {!topProfiles.length ? (
                <div className="text-xs text-slate-400">No wallet profiles yet.</div>
              ) : null}
            </div>
          </div>
        </aside>
