...
export default function DaoAgentCouncil({
  items,
}: {
  items: DaoAgentRecommendation[];
}) {
  if (!items.length) {
    return (
      <p className="muted-line">No agent recommendations for this view.</p>
    );
  }
  return (
      ...
      {items.map((a) => (
        <div key={a.id} className="item-card">
          ...
          <div className="item-meta">
            ...
            <span className="mini-pill">
              human: {a.humanDisposition ?? "pending"}
            </span>
          ...
          {a.storageRef ? (
            <div className="muted-line">off-chain {a.storageRef}</div>
          ) : null}
        </div>
      ))}
    </div>
