...
export default function DaoMemoryPanel({
  items,
}: {
  items: DaoGovernanceMemoryRecord[];
}) {
  if (!items.length)
    return (
      <p className="muted-line">
        Governance memory will appear after finalize / execute cycles.
      </p>
    );
  return (
        ...
        <div key={m.id} className="item-card">
          ...
          {m.linkedReceiptIds.length ? (
            <div className="muted-line">
              receipts: {m.linkedReceiptIds.join(", ")}
            </div>
          ) : null}
          {m.storageRef ? (
            <div className="muted-line">off-chain {m.storageRef}</div>
          ) : null}
        </div>
      ))}
    </div>
