...
export default function DaoVotePanel({
  ...
  const open =
    proposal.status === "voting" || proposal.status === "quorum_reached";
    ...
    <div>
      ...
      {!walletConnected ? (
        <p className="muted-line">
          Connect a Solana wallet to vote. Identity is the wallet — not
          localStorage.
        </p>
      ) : effectiveWeight <= 0 ? (
        <p className="muted-line">
          No effective voting power (delegate inactive, or all power delegated
          away). Revoke delegation or receive delegation to vote.
        </p>
      ) : !open ? (
        <p className="muted-line">
          Voting closed for status {proposal.status}.
        </p>
      ) : (
