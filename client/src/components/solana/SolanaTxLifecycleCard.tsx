...
const VARIANTS: Partial<Record<WalletConnectionStatus, { variant: string }>> = {
  ...
  session_verifying: {
    variant: "border-amber-400/35 bg-amber-500/15 text-amber-50",
  },
  session_verified: {
    variant: "border-[#3bff96]/45 bg-[#3bff96]/12 text-[#d8ffe9]",
  },
  wrong_cluster: {
    variant: "border-amber-400/50 bg-amber-500/15 text-amber-50",
  },
...
export function SolanaTxLifecycleCard({
  status,
}: {
  status: WalletConnectionStatus;
}) {
  const variant =
    VARIANTS[status]?.variant ?? "border-white/15 bg-white/5 text-slate-200";
  ...
  return (
    <div className={cn("rounded-xl border border-white/10 bg-black/35 p-4")}>
      <p className="text-xs uppercase tracking-[0.14em] text-slate-500">
        {SOLANA_COPY.wallet.lifecycleTitle}
      </p>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <Badge className={cn("text-[11px] font-semibold", variant)}>
          {label}
        </Badge>
      </div>
    </div>
