...
import { artifactExplorerUrl, computeExplorerUrl, daExplorerUrl } from "@/lib/zerog/explorer";
...
const LAYOUT: Array<{ id: string; label: string; role: string; x: number; y: number }> = [
  { id: "wallet", label: "Solana wallet", role: "identity + session", x: 16, y: 24 },
  ...
  { id: "reflection", label: "Reflection", role: "full payload", x: 552, y: 24 },
  { id: "memory", label: "Memory", role: "next-turn linkage", x: 16, y: 132 },
  { id: "storage", label: "0G storage", role: "durable artifact", x: 150, y: 132 },
  { id: "compute", label: "0G compute", role: "summarize / normalize", x: 284, y: 132 },
  { id: "da", label: "0G DA", role: "availability ref", x: 418, y: 132 },
  { id: "receipt", label: "Solana receipt", role: "compact proof", x: 552, y: 132 },
  { id: "explorer", label: "Verify", role: "explorer + replay", x: 360, y: 220 },
];
...
export function ZeroGProofGraph({ graph }: { graph: ZeroGProofGraphResponse | null }) {
  ...
  const storageUrl = latestArtifact?.storageRef ? artifactExplorerUrl(config, latestArtifact.storageRef) : undefined;
  const computeUrl = latestJob?.computeRef ? computeExplorerUrl(config, latestJob.computeRef) : undefined;
  const daUrl = latestDa?.availabilityRef ? daExplorerUrl(config, latestDa.availabilityRef) : undefined;

  return (
    <section className="rounded-2xl border border-white/10 bg-[#060b10]/95 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-semibold text-white">Solana proof layer + 0G modular sidecar</h3>
        <span className="text-[10px] uppercase tracking-[0.14em] text-slate-500">replayable · hash-linked</span>
      </div>
      <p className="mt-1 text-[11px] text-slate-400">
        Heavy narrative and artifacts: <span className="text-[#7dffc4]">0G</span>. Canonical identity and compact proofs:{" "}
        <span className="text-[#7dffc4]">Solana</span>.
      </p>
          ...
          {EDGES.map(edge => {
            ...
            return (
              <g key={`${edge.from}-${edge.to}`}>
                <line x1={a.cx} y1={a.cy} x2={b.cx} y2={b.cy} stroke="url(#zg-edge)" strokeWidth={1.25} />
                <text
                  x={(a.cx + b.cx) / 2}
                  y={(a.cy + b.cy) / 2 - 4}
            ...
            return (
              <g key={node.id} transform={`translate(${node.x},${node.y})`}>
                <rect
                  ...
                  height={NODE_H}
                  rx={10}
                  fill={active ? "rgba(59,255,150,0.08)" : "rgba(15,23,42,0.65)"}
                  stroke={active ? "rgba(59,255,150,0.45)" : "rgba(255,255,255,0.12)"}
                  strokeWidth={1}
                />
                <text x={12} y={20} fill="#e2e8f0" fontSize={11} fontWeight={600}>
                  {node.label}
                </text>
      ...
      <div className="mt-4 grid gap-2 rounded-xl border border-white/10 bg-black/40 p-3 text-[11px] text-slate-300 md:grid-cols-2">
        <div>
          <p className="text-[10px] uppercase tracking-[0.12em] text-slate-500">Latest link</p>
          <p>status: {latestLink?.status || "unlinked"}</p>
          <p>content hash: {latestLink ? formatHash(latestLink.contentHash, 6) : "—"}</p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-[0.12em] text-slate-500">0G refs</p>
          <p>storage: {latestArtifact?.storageRef || "—"}</p>
          <p>compute: {latestJob?.computeRef || "—"}</p>
          <p>DA: {latestDa?.availabilityRef || "—"}</p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-[0.12em] text-slate-500">Solana proof</p>
          <p>tx: {latestReceipt?.txSignature ? formatHash(latestReceipt.txSignature, 8) : "not anchored"}</p>
          <p>summary hash: {latestReceipt ? formatHash(latestReceipt.summaryHash, 6) : "—"}</p>
        </div>
        <div className="flex flex-col gap-1">
          <p className="text-[10px] uppercase tracking-[0.12em] text-slate-500">Open explorers</p>
          <div className="flex flex-wrap gap-2">
            ...
            {daUrl ? (
              <a href={daUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#78f4e1] hover:underline">
