...
export function DemoEventLogPanel({
  presentationMode,
}: {
  ...
  return (
    <DemoPanel
      ...
      className="max-h-[420px] space-y-3 overflow-y-auto"
    >
      <div>
        ...
        <p className="mt-1 text-xs text-slate-500">
          Deterministic log · {log.length} events · posture{" "}
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span
                  className={cn(
                    "font-medium",
                    statusTone[ev.status] ?? "text-slate-200",
                  )}
                >
                  {ev.label}
                </span>
              ...
              {ev.linkUrl ? (
                <div className="mt-2">
                  <ExplorerLinkButton
                    payload={{ label: "Open link", url: ev.linkUrl }}
                  />
                </div>
              ) : null}
              {ev.notes ? (
                <p className="mt-1 text-[10px] text-slate-600">{ev.notes}</p>
              ) : null}
            </li>
          ))}
      </ul>
