    ...
    >
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-4">
          ...
          <dl className="mt-4 grid gap-2 text-[11px] text-slate-400 sm:grid-cols-2">
            <div className="flex justify-between gap-2 border border-white/[0.06] bg-black/30 px-3 py-2 rounded-lg">
              ...
              <dd className="truncate font-mono text-slate-200">
                {run.walletAddress
                  ? `${run.walletAddress.slice(0, 4)}…${run.walletAddress.slice(-4)}`
                  : "unknown"}
              </dd>
            </div>
            <div className="flex justify-between gap-2 border border-white/[0.06] bg-black/30 px-3 py-2 rounded-lg">
              <dt>Skill</dt>
              <dd className="truncate text-[#cfefff]">
                {run.skillName ?? "unknown"} · v{run.skillVersion ?? "?"}
              </dd>
            </div>
          </dl>
        </div>
        <div className="flex flex-col items-end gap-2">
          {(run.metadata.demoOnly ?? run.metadata.demo_only) ? (
            <StatusChip
              tone="warn"
              label="Demo mode labeling · deterministic fixtures"
            />
          ) : null}
          <StatusChip
            tone="live"
            label={`Execution stage · ${run.currentStage}`}
          />
          <StatusChip
            tone={run.failureReason ? "warn" : "proof"}
            label={
              run.failureReason ?? "No plan step failure on this scrub frame"
            }
          />
        </div>
      </div>
          ...
          <div
            ...
            className={cn(
              ...
              run.activeStepId === step.id
                ? "border-[#14f195]/55 bg-[#14f195]/08 shadow-[0_0_28px_rgba(20,241,149,0.12)]"
          ...
          >
            ...
            {step.agentName ? (
              <p className="mt-1 text-[10px] text-slate-500">
                Delegation ·{" "}
                <span className="text-slate-200">{step.agentName}</span>
              </p>
            ) : (
              <p className="mt-1 text-[10px] text-slate-600">
                Delegation · unknown (no agent mapping)
              </p>
            )}
            ...
            {step.toolCalls?.length ? (
              <div className="mt-3 space-y-2 border-l-2 border-white/15 pl-3">
                {(step.toolCalls ?? []).map((tc) => (
                  <div
                    key={tc.id}
                    className="rounded-lg border border-white/8 bg-black/40 px-3 py-2"
                  >
                    ...
                    {tc.outputSummary ? (
                      <p className="mt-1 text-[11px] text-[#cfefff]/90">
                        {tc.outputSummary}
                      </p>
                    ) : null}
                    <div className="mt-2">
      ...
      {run.failureReason ? (
        ...
        <p className="relative mt-3 text-[11px] text-slate-600">
          Failure reason · unknown (no surfaced operator fault on this scrub
          frame).
        </p>
      )}
    </DemoPanel>
