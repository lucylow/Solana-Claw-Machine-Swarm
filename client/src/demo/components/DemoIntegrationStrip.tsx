...
export function DemoIntegrationStrip({
  presentationMode,
}: {
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2">
      <DemoPanel
        ...
        className="space-y-2 border-cyan-500/20"
      >
        <p className="text-[10px] font-semibold uppercase tracking-wider text-cyan-200/90">
          0G storage / DA (demo)
        </p>
        <div className="flex flex-wrap gap-2">
          ...
          >
            Storage · {z?.storage.available ? "available" : "unavailable"} ·{" "}
            {z?.mode ?? "—"}
          </Badge>
          <Badge
            variant="outline"
        ...
        <p className="font-mono text-[10px] text-slate-500">
          Batch root · {z?.da.lastRootHash ?? "not committed on this beat"}
        </p>
      </DemoPanel>
      <DemoPanel
        presentationMode={presentationMode}
        className="space-y-2 border-violet-500/20"
      >
