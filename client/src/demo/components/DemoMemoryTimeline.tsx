...
export function DemoMemoryTimeline({
  stages,
}: {
  stages: DemoMemoryTimelineStage[];
}) {
  return (
    <DemoPanel className="space-y-4">
      ...
      <h3 className="text-lg font-semibold text-white">
        From capture to verified reuse
      </h3>
          ...
          <div key={st.id} className="relative flex gap-4 pb-8 last:pb-0">
            <div
              className={cn(
                ...
                st.status === "complete" &&
                  "border-[#3bff96] bg-[#3bff96]/30 shadow-[0_0_10px_rgba(59,255,150,0.35)]",
                st.status === "active" &&
                  "border-[#38d7d0] bg-[#38d7d0]/40 animate-pulse",
            ...
            <div className="flex-1 rounded-xl border border-white/10 bg-black/35 p-3">
              ...
              <p className="mt-2 text-[11px] text-slate-600">
                {new Date(st.timestampIso).toLocaleString()}
              </p>
              {st.proofOrStorageRef ? (
                <p className="mt-1 break-all font-mono text-[10px] text-[#8ceada]">
                  {st.proofOrStorageRef}
                </p>
              ) : null}
            </div>
