...
export function DemoOrchestrationFlow({
  agents,
}: {
  agents: DemoAgentFixture[];
}) {
  const order: DemoAgentFixture["role"][] = [
    "planner",
    "researcher",
    "operator",
    "critic",
  ...
  const sorted = order
    ...
    .filter(Boolean) as DemoAgentFixture[];
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <h3 className="text-lg font-semibold text-white">
            Lanes · merge · coordinator verdict
          </h3>
        </div>
      </div>
      <div className="flex flex-col gap-2 lg:flex-row lg:flex-wrap lg:items-stretch">
        {sorted.map((agent, idx) => (
          ...
          >
            <div
              className={cn(
                "w-full min-w-[200px] max-w-sm lg:w-[220px]",
                agent.role === "coordinator" &&
                  "ring-1 ring-[#3bff96]/30 rounded-2xl",
              )}
            >
              <DemoAgentCard
                agent={agent}
                active={
                  agent.role === "coordinator" || agent.status === "blocked"
                }
              />
            </div>
            {idx < sorted.length - 1 ? (
              <ArrowDown
                className="h-5 w-5 shrink-0 text-slate-600 lg:hidden"
                aria-hidden
              />
            ) : null}
            {idx < sorted.length - 1 ? (
              <span
                className="hidden px-1 text-slate-600 lg:inline"
                aria-hidden
              >
                →
              </span>
