...
export function DemoPlanCard({ plan, presentationMode, glow }: { plan: DemoPlanFixture; presentationMode?: boolean; glow?: boolean }) {
  return (
    <DemoPanel glow={glow} presentationMode={presentationMode} className="space-y-4">
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <h3 className={cn("font-semibold text-white", presentationMode ? "text-xl" : "text-lg")}>{plan.goal}</h3>
      ...
      <div className="grid gap-2 text-sm md:grid-cols-2">
        ...
        <div className="rounded-lg border border-white/10 bg-black/35 p-2">
          ...
          <p className="mt-1 text-slate-200">{plan.stepCount} with dependency graph</p>
        </div>
        ...
        <div className="rounded-lg border border-white/10 bg-black/35 p-2 md:col-span-2">
          ...
          <p className="mt-1 font-mono text-xs text-[#c8ffe2]">{plan.chosenSkillIds.join(", ")}</p>
        </div>
        <div className="rounded-lg border border-white/10 bg-black/35 p-2">
          <p className="text-xs text-slate-500">Plan hash</p>
          <p className="mt-1 break-all font-mono text-[11px] text-slate-400">{plan.planSummaryHash}</p>
        </div>
        <div className="rounded-lg border border-white/10 bg-black/35 p-2">
        ...
        <div className="rounded-lg border border-white/10 bg-black/35 p-2 md:col-span-2">
          ...
          <p className="mt-2 break-all font-mono text-[11px] text-slate-500">Result hash: {plan.resultHash}</p>
        </div>
      </div>
    </DemoPanel>
  );
