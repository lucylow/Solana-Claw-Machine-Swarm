...
export function DemoPlaybackController({
  presentationMode,
}: {
    ...
    <div className="space-y-4">
      <DemoPanel presentationMode={presentationMode} className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#87f7d0]">
            Scenario
          </p>
        </div>
        ...
        <p className="text-[11px] text-slate-500">
          {DEMO_SCENARIOS.find((s) => s.id === selectedScenarioId)?.summary}
        </p>
      </DemoPanel>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        >
          ...
          <ul className="space-y-1 text-[11px] text-slate-400">
            <li>
              Wallet verified ·{" "}
              {demoSnapshot.derived.isWalletVerified ? "yes" : "no"}
            </li>
            <li>
              Proof verified (strict) ·{" "}
              {demoSnapshot.derived.isProofVerified
                ? "yes"
                : "no — demo receipts"}
            </li>
            <li>
              Memory linked ·{" "}
              {demoSnapshot.derived.isMemoryLinked ? "yes" : "no"}
            </li>
            <li>
              Failure / recovery ·{" "}
              {demoSnapshot.derived.hasFailure ? "failure path" : "clean"} /{" "}
              {demoSnapshot.derived.hasRecovery ? "recovery arc" : "—"}
            </li>
