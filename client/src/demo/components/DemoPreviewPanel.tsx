...
export function DemoPreviewPanel({
  ...
  const activePlaybackStep =
    displayedExecutionRun.steps.find(
      (s) => s.id === displayedExecutionRun.activeStepId,
    ) ?? displayedExecutionRun.steps.slice(-1)[0];

  const legacyActiveStep =
    steps.find((s) => s.status === "active" || s.status === "failed") ??
  ...
  return (
    ...
    >
      ...
      <div className="rounded-xl border border-white/10 bg-black/35 p-3 text-[11px] text-slate-400">
        ...
        <p className="mt-1 text-[#dffefc]">
          {displayedExecutionRun.currentStage}
        </p>
        <p className="mt-2 font-mono text-[10px] text-slate-500">
          {displayedExecutionRun.id}
        </p>
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <div
          className={cn(
            ...
            highlightClass(guidedActive("wallet")),
          )}
        >
          ...
          <p className="mt-1 text-xs text-slate-500">
            {demoSnapshot.wallet.cluster} cluster ·{" "}
            {demoSnapshot.derived.dataPosture.replace(/_/g, " ")}
          </p>
        </div>
        ...
        <div
          className={cn(
            "rounded-xl border p-3 transition-colors",
            highlightClass(guidedActive("skills")),
          )}
        >
          <div className="mb-2 flex items-center gap-2 text-xs text-slate-400">
          ...
          <p className="mt-1 text-xs text-slate-500">
            v{activeSkill.version} · rep {activeSkill.reputationScore}
          </p>
        </div>
        ...
        <div
          className={cn(
            "rounded-xl border p-3 transition-colors",
            highlightClass(guidedActive("plan")),
          )}
        >
          <div className="mb-2 flex items-center gap-2 text-xs text-slate-400">
        ...
        <div
          className={cn(
            "rounded-xl border p-3 transition-colors",
            highlightClass(guidedActive("execution")),
          )}
        >
          <div className="mb-2 flex items-center gap-2 text-xs text-slate-400">
          ...
          <p className="text-sm text-white">
            {activePlaybackStep?.title ?? legacyActiveStep?.title}
          </p>
          <p className="mt-1 text-xs text-slate-500">
            {activePlaybackStep?.description ?? legacyActiveStep?.detail}
          </p>
        </div>
        ...
        <div
          className={cn(
            "rounded-xl border p-3 transition-colors",
            highlightClass(guidedActive("coordination")),
          )}
        >
          <div className="mb-2 flex items-center gap-2 text-xs text-slate-400">
          ...
          <p className="mt-2 text-xs text-slate-300">
            {displayedExecutionRun.activeAgentRole ??
              coordinator?.outputSummary ??
              "unknown"}
          </p>
        </div>
        ...
        <div
          className={cn(
            "rounded-xl border p-3 transition-colors",
            highlightClass(guidedActive("reputation")),
          )}
        >
          <div className="mb-2 flex items-center gap-2 text-xs text-slate-400">
          ...
          <p className="text-xs text-slate-300">
            Success {activeSkill.successRate}% · usage{" "}
            {activeSkill.usageCount.toLocaleString()} · last{" "}
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div
          className={cn(
            ...
            highlightClass(guidedActive("reflection")),
          )}
        >
          ...
          <p className="text-xs font-mono text-slate-300">
            {displayedExecutionRun.reflectionId ??
              reflection?.id ??
              "hidden / unavailable on this scrub"}
          </p>
        </div>
        <div
          className={cn(
            "rounded-xl border p-3",
            highlightClass(guidedActive("memory")),
          )}
        >
          <div className="mb-1 flex items-center gap-2 text-xs text-slate-400">
          ...
          <p className="text-xs font-mono text-slate-300">
            {displayedExecutionRun.memoryId ?? memory?.id ?? "episodic only"}
          </p>
        </div>
        <div
          className={cn(
            "rounded-xl border p-3",
            highlightClass(guidedActive("receipt")),
          )}
        >
          <div className="mb-1 flex items-center gap-2 text-xs text-slate-400">
          ...
          <p className="text-xs font-mono text-slate-300">
            {displayedExecutionRun.proofId ??
              displayedExecutionRun.receiptId ??
              primaryReceipt?.id}
          </p>
          <div className="mt-2">
            <ExplorerLinkButton
      ...
      {guidedStep ? (
        <p className="border-t border-white/10 pt-3 text-xs text-slate-500">
          Playback focus ·{" "}
          <span className="font-mono text-slate-300">{guidedStep.id}</span>:{" "}
