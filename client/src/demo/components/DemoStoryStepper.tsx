  ...
  return (
    <DemoPanel presentationMode={presentationMode} className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-xs uppercase tracking-[0.14em] text-[#87f7d0]">
            Demo story controller
          </p>
          <p className="text-sm font-medium text-white">
            Step {storyPlaybackIndex + 1} / {storyBeatCount} ·{" "}
            {selectedScenarioId.replace(/-/g, " ")}
          </p>
          <p className="mt-1 text-[10px] font-mono text-slate-500">
            Outcome rail · {runOutcome} · data posture ·{" "}
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          <Button
            ...
            className="border-white/15 text-slate-200"
            onClick={() =>
              setStoryPlaybackIndex((i) => Math.min(storyBeatCount - 1, i + 1))
            }
          >
            Next
      ...
      <div className="rounded-xl border border-[#14f195]/25 bg-black/40 p-3">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-[#14f195]/90">
          {step.patch.currentStage}
        </p>
        <p className="mt-2 text-base font-semibold text-[#d4ffef]">
          {step.title}
        </p>
        ...
        {showPresenterNotes ? (
          <p className="mt-3 border-t border-white/10 pt-3 text-xs text-amber-100/90">
            <span className="font-medium text-amber-50">Presenter:</span>{" "}
            {step.presenterNote}
          </p>
        ) : null}
      </div>
      ...
      <div className="flex items-center gap-2 text-[10px] uppercase tracking-wide text-slate-600">
        ...
        <span>
          Jump rail follows scenario frames — receipts stay explicit about demo
          vs verified.
        </span>
      </div>
    </DemoPanel>
