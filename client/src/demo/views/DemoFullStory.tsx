...
function executionStepsToTimelineFixtures(
...
export function DemoFullStory() {
  ...
  const glow = (region: (typeof currentBeat)["highlight"]) =>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Wallet connect → skill choose → plan → execute → reflection → memory
            → receipt → verified next-turn reuse.
          </p>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="space-y-4">
          <DemoExecutionRunPanel
            run={displayedExecutionRun}
          ...
          <DemoPanel className="border-[#38d7d0]/20 p-4 text-sm text-slate-300">
            <p className="text-xs uppercase tracking-wide text-[#87f7d0]">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <DemoReflectionCard
              reflection={
                displayedExecutionRun.reflectionId ? reflection : null
              }
              presentationMode={presentationMode}
              glow={glow("reflection")}
            />
            <DemoMemoryCard
              memory={displayedExecutionRun.memoryId ? memory : null}
              presentationMode={presentationMode}
              glow={glow("memory")}
            />
          </div>
          {traceableMemory && displayedExecutionRun.memoryId ? (
            <DemoTraceableMemoryPanel
              memory={traceableMemory}
              presentationMode={presentationMode}
              beatHighlight={currentBeat.highlight}
            />
          ) : null}
          ...
          <div className="space-y-2">
              ...
              <DemoPanel className="border-white/15 p-4">
                ...
                <ul className="mt-3 space-y-2 text-[11px] text-slate-300">
                  {visibleCommandReceipts.map((cr) => (
                    ...
                    >
                      <span className="font-mono text-[#bfffea]">
                        {cr.type}
                      </span>{" "}
                      · <span>{cr.title}</span>
                      <p className="mt-1 font-mono text-[10px] text-slate-500">
                        {cr.claim.proofState.replace(/_/g, " ")}
                      </p>
                      {cr.demoLabeled ? (
                        <p className="mt-1 text-[10px] uppercase tracking-wide text-amber-200">
                          Demo-labeled deterministic anchor
                        </p>
                      ) : null}
                    </li>
                  ))}
            ...
            {visibleReceipts.map((r: DemoReceiptFixture, i: number) => (
              <DemoReceiptCard
                key={r.id}
                receipt={r}
                defaultOpen={i === visibleReceipts.length - 1}
              />
            ))}
          </div>
