...
export function DemoHub() {
  const { selectedScenarioId, setSelectedScenarioId, presentationMode } =
    useDemo();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-[#8ceada]">
            CLAW_MACHINE · demo command center
          </p>
          <h1
            className={cn(
              "mt-2 font-semibold text-white",
              presentationMode ? "text-4xl" : "text-3xl md:text-4xl",
            )}
          >
            Interactive mock narrative
          </h1>
          <p className="mt-3 max-w-2xl text-sm text-slate-400 md:text-base">
            A presenter-ready, Solana-default loop: wallet connect, skill
            discovery, multi-agent execution, reflection, durable memory, and
            receipts anyone can verify on Solana Explorer — without waiting on
            live chain state.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_1.1fr]">
        <div className="space-y-4">
          <DemoPanel className="space-y-4">
            <div className="flex flex-wrap gap-2">
              ...
              <Link href="/demo/playground">
                <Button
                  variant="outline"
                  className="border-[#38d7d0]/50 text-[#d4fffb]"
                >
            ...
            <p className="text-xs text-slate-500">
              Guided mode includes autoplay, presenter notes, and step jump.
              Playground lets you flip success, failure, and recovery without
              leaving the page.
            </p>
          </DemoPanel>
          ...
          <DemoPanel className="space-y-2">
            <h3 className="text-sm font-medium text-white">
              What you will see · {selected.title}
            </h3>
        ...
        <div>
          ...
          <p className="mt-2 text-center text-[11px] text-slate-600">
            Selecting a scenario updates this preview — open{" "}
            <span className="text-slate-400">Guided full story</span> for
