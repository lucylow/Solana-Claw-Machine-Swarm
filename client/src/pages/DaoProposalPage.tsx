      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-black/70 backdrop-blur">
        <div className="container flex flex-wrap items-center justify-between gap-3 py-4">
          <div className="flex items-center gap-3">
            <Link
              href="/dao"
              className="ghost-btn inline-flex items-center text-[#b8ffe0]"
            >
        ...
        {!data || !p ? (
          ...
          <section className="dao-shell mt-6">
            <div className="item-meta mb-4">
              ...
              <span className="mini-pill">
                cluster {p.onchain?.cluster ?? data.cluster}
              </span>
            </div>
            ...
            <div
              className="dao-cc-nav mt-4"
              style={{ position: "static", flexWrap: "wrap" }}
            >
              {(
                [
                  "summary",
            ...
            <div className="mt-4">
              {tab === "summary" ? (
                <div>
                  ...
                  {p.fullDescription ? (
                    <p className="muted-line mt-2">{p.fullDescription}</p>
                  ) : null}
                  <div className="dao-cc-split mt-4">
                    ...
                    <div className="item-card">
                      ...
                      <div className="muted-line">
                        yes {p.voteYes} · no {p.voteNo} · abstain{" "}
                        {p.voteAbstain} · veto {p.voteVeto}
                      </div>
                    </div>
                  </div>
                  <div className="item-card mt-3">
                    ...
                    <div className="muted-line">
                      ref {p.offchain?.storageRef ?? "—"}
                    </div>
                    <div className="muted-line">
                      thread {p.offchain?.discussionThreadId ?? "—"}
                    </div>
                  </div>
                </div>
              ) : null}
                ...
                <div className="stack-list">
                  {data.delegations.map((d) => (
                    <div key={d.id} className="item-card">
                      ...
                      <div className="muted-line">
                        status {d.status} · weight {d.weight}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {tab === "agents" ? (
                <DaoAgentCouncil
                  items={data.agentRecommendations.filter(
                    (a) => a.proposalId === id,
                  )}
                />
              ) : null}
              ...
              {tab === "treasury" ? (
                <div className="item-card">
                  {p.treasuryImpact ? (
                    <>
                      <div className="text-white">
                        {(p.treasuryImpact.amount ?? 0) / 1e9} SOL ·{" "}
                        {p.treasuryImpact.budgetCategory}
                      </div>
                      <div className="muted-line font-mono text-xs">
                        {p.treasuryImpact.destination}
                      </div>
                    </>
                  ) : (
                    <p className="muted-line">
                      No direct treasury impact on this proposal type.
                    </p>
                  )}
                </div>
              ) : null}
              ...
              {tab === "receipts" ? (
                <div className="item-card">
                  <div className="muted-line">
                    proposal receipt {p.proposalReceiptId ?? "—"}
                  </div>
                  <div className="muted-line">
                    execution {p.executionReceiptId ?? "—"}
                  </div>
                  ...
                  {p.onchain?.pda ? (
                    <div className="muted-line mt-2">PDA {p.onchain.pda}</div>
                  ) : null}
                </div>
              ) : null}
              ...
              {tab === "memory" ? (
                <DaoMemoryPanel
                  items={data.governanceMemory.filter(
                    (m) => m.proposalId === id,
                  )}
                />
              ) : null}
