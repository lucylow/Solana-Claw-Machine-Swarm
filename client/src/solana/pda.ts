...
function programKey(programId?: string) {
  const value = (programId || CLAW_ONCHAIN_PROGRAM_ID || DEFAULT_PROGRAM_ID).trim();
  try {
...
export function deriveConfigPda(programId?: string) {
  ...
  const [pda] = PublicKey.findProgramAddressSync([Buffer.from(CONFIG_SEED)], pid);
  return pda.toBase58();
}

export function deriveProfilePda(walletAddress: PublicKey | string, programId?: string) {
  const pid = programKey(programId);
  const wallet = new PublicKey(normalizeWalletAddress(walletAddress));
  const [pda] = PublicKey.findProgramAddressSync([Buffer.from(PROFILE_SEED), wallet.toBuffer()], pid);
  return pda.toBase58();
}
...
export function deriveSkillPda(
  ...
  const [pda] = PublicKey.findProgramAddressSync(
    [Buffer.from(SKILL_SEED), wallet.toBuffer(), Buffer.from(String(slug).trim().toLowerCase())],
    pid
  );
  return pda.toBase58();
}
...
export function deriveSkillVersionPda(
  ...
  const [pda] = PublicKey.findProgramAddressSync(
    [Buffer.from(SKILL_VERSION_SEED), skill.toBuffer(), Buffer.from(String(version).trim())],
    pid
  );
  return pda.toBase58();
}
