...
export function asyncRoute(handler: AsyncRoute, routeLabel?: string): AsyncRoute {
  return async (req, res, next) => {
    try {
      ...
      logStructuredError(appError, { route: req.path, requestId: String(req.headers["x-request-id"] || "") });
      if (!res.headersSent) {
