...
export function getZeroGConfig(): ZeroGConfig {
  ...
  const ogChainId = Number(
    process.env.ZEROG_OG_CHAIN_ID || ZEROG_CHAIN_ID_DEFAULT,
  );
  return {
    environment,
    storageUrl:
      process.env.ZEROG_STORAGE_URL || "https://storage.demo.0g.ai/v1",
    computeUrl:
      process.env.ZEROG_COMPUTE_URL || "https://compute.demo.0g.ai/v1",
    dataAvailabilityUrl: process.env.ZEROG_DA_URL || "https://da.demo.0g.ai/v1",
    explorerUrl:
      process.env.ZEROG_EXPLORER_URL || "https://explorer.demo.0g.ai",
    ...
    bridgeProvider:
      process.env.ZEROG_BRIDGE_PROVIDER || "XSwap (per official 0G docs)",
