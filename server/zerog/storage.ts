...
import type { ZeroGHealthStatus, ZeroGStorageAdapter, ZeroGStorageArtifact } from "./types";
...
export class ZeroGStorageService implements ZeroGStorageAdapter {
  ...
  async storeArtifact(input: ZeroGStorageArtifact): Promise<ZeroGStorageArtifact> {
    const config = getZeroGConfig();
    const contentHash = input.contentHash || hashValue(input.content);
    const next: ZeroGStorageArtifact = {
      ...input,
      contentHash,
      checksum: input.checksum || hashValue({ id: input.id, contentHash, title: input.title }),
  ...
  async verifyArtifact(storageRef: string, expectedHash: string): Promise<boolean> {
  ...
  async listArtifactsByKind(kind: ZeroGStorageArtifact["kind"]): Promise<ZeroGStorageArtifact[]> {
    return this.store
      .listArtifacts()
