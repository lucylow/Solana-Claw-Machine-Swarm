...
describe("buildDemoExecutionArtifacts + story playback", () => {
  it("hydrates lineage fields for recovery outcome", () => {
    const skill = DEMO_SKILLS.find(s => s.id === "skill-support-triage") ?? DEMO_SKILLS[0]!;
    const plan = buildPlan(skill, "recovery");
    const steps = buildExecutionSteps("recovery");
    const receipts = buildReceipts(skill, "recovery");

    const { executionRun, reflection, traceableMemory, commandReceipts } = buildDemoExecutionArtifacts({
      wallet: DEMO_WALLET,
      skill,
      plan,
      stepFixtures: steps,
    ...
    const operatorStep = executionRun.steps.find(s => s.title.includes("Execute primary"));
