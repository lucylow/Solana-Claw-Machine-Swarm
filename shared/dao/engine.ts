...
export function participationRatio(
  voterCount: number,
...
export function participationBpsFromCounts(
  voterCount: number,
  eligibleMembers: number,
): number {
...
export function deriveCanonicalStatus(input: {
  ...
  if (input.legacyStatus === "succeeded")
    return input.executed ? "executed" : "approved";
  if (input.legacyStatus === "defeated") return "rejected";
  if (!input.finalized && input.legacyStatus === "active") {
    if (quorumMetBps(input.participationBps, input.quorumBps))
...
export function proposalExecutionReady(input: {
  ...
  if (input.status !== "approved" && input.status !== "quorum_reached")
    return false;
  return (
