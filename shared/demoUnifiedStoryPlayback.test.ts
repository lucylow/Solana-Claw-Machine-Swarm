...
import { applyStoryPlayback, getUnifiedStoryBeats, SUCCESS_BEATS } from "./demoUnifiedStoryPlayback";
...
describe("demo unified story playback", () => {
  ...
  it("clears failure copy on retry beat after injecting memory", () => {
    ...
    const retryBeat = getUnifiedStoryBeats(outcome).find(b => b.id === "rv-12-retry");
